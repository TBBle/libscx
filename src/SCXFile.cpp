...
using scene_blobs_writeable_span = span<byte, dynamic_range, Scene::blob_size>;
...
void write_scene_data(const scene_storage_data& scene_data,
                      ...
                      span<uint32_t> scene_text_offsets,
                      span<byte> scene_text_storage, uint32_t base_offset) {
...
using fixed_strings_writeable_span =
    span<byte, dynamic_range, fixed_string_size>;
...
using variable_blobs_writeable_span =
    span<byte, dynamic_range, Variable::blob_size>;
using fixed_string_pairs_writeable_span =
    span<byte, dynamic_range, 2, fixed_string_size>;
...
bool SCXFile::read(const string& fileName) {
  ...
  variable_blobs_span variable_blobs =
      as_span(buffer.first(Variable::blob_size *
  ...
  fixed_strings_span table1_string_buffers = as_span(
  ...
  fixed_string_pairs_span variable_strings_buffers =
      as_span(buffer.subspan(header.offsets[SCXFileHeader::variable],
                             fixed_string_size * 2 *
                                 header.counts[SCXFileHeader::variable]),
...
#define READ_ASSET_STRINGS(ASSETTYPE, STORAGE)                          \
  fixed_string_pairs_span ASSETTYPE##_strings_buffers =                 \
      as_span(buffer.subspan(header.offsets[SCXFileHeader::ASSETTYPE],  \
...
bool SCXFile::write(const string& fileName) {
  ...
  auto scene_string_offsets =
      as_span<uint32_t>(buffer.first(sizeof(uint32_t) * header.scene_count));
  buffer = buffer.subspan(scene_string_offsets.size_bytes());
  ...
  auto scene_text_blob = as_span(buffer.first(scene_text_size_total),
  ...
  fixed_strings_writeable_span table1_string_buffers = as_span(
  ...
  header.offsets[SCXFileHeader::variable] =
      narrow_cast<uint32_t>(storage.size() - buffer.size_bytes());
  fixed_string_pairs_writeable_span variable_strings_buffers =
      as_span(buffer.first(fixed_string_size * 2 *
                           header.counts[SCXFileHeader::variable]),
              dim<>(header.counts[SCXFileHeader::variable]), dim<2>(),
              dim<fixed_string_size>());
